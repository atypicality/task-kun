...
async function callGemini(userMessage: string, taskContext: string): Promise<string> {
  ...
  const response = await fetch("http://127.0.0.1:8000/generate-gemini", {
    method: "POST",
...
export function AIPage() {
  ...
  const getTaskContext = () => {
    ...
    if (tasks.length === 0) return "User has no active tasks.";
    return `User has ${tasks.length} active tasks. Top tasks: ${tasks.slice(0, 3).map((t) => `"${t.title}" (${t.priority} priority, due ${t.dueDate || "unknown"})`).join(", ")}.`;
  };
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      // Generate and play voice for the response
      try {
        ...
        // Continue without audio - don't interrupt the chat experience
      }
    } catch {
              ...
              <div className="ai-contextCard">
                ...
                <div className="ai-contextBody">
                  {(() => {
                    ...
                    if (active.length === 0) return <div className="ai-noTasks">No active tasks yet. Add some from the dashboard!</div>;
                    return active.slice(0, 5).map((t) => (
                      <div key={t.id} className="ai-contextTask">
                        <div className={`ai-contextPriority ${t.priority}`} />
                        <div>
                          <div className="ai-contextTaskTitle">{t.title}</div>
                          {t.dueDate && <div className="ai-contextTaskMeta">Due {t.dueDate}</div>}
                        </div>
                      </div>
                    ));
                  })()}
